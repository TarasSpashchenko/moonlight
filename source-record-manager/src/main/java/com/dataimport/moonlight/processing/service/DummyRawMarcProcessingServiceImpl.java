...
public class DummyRawMarcProcessingServiceImpl implements RawMarcProcessingService {
  private static final Logger LOGGER = LoggerFactory.getLogger(DummyRawMarcProcessingServiceImpl.class);
  ...
  private final DummyReadThroughCache readThroughCache = new DummyReadThroughCache() {
    ...
    public Future<JsonObject> readSomeData(JsonObject someParams) {
      ...
      return (random.nextInt(10000) > 1000) ? Future.succeededFuture(new JsonObject()) : someDataDao.readSomeData(someParams);
    }
  };
  ...
  public DummyRawMarcProcessingServiceImpl(Vertx vertx) {
    this.vertx = vertx;
  }
